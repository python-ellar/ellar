...
from pydantic import AnyUrl, EmailStr, BaseModel
from pydantic.fields import ModelField
from pydantic.schema import get_model_name_map, model_process_schema, get_flat_models_from_fields
...
from ..schema import ValidationError, HTTPValidationError
...
class OpenAPIDocumentBuilder:
    def __init__(self) -> None:
        self._build = dict()
        self._build.setdefault('info', {}).update(title='StarletteAPI Docs', version='1.0.0')
    ...
    def set_contact(
            ...
            email: t.Optional[EmailStr] = None) -> 'OpenAPIDocumentBuilder':
        self._build['info']['contact'] = dict(name=name, url=url, email=email)
        return self

    def set_license(self, name: str, url: t.Optional[AnyUrl] = None) -> 'OpenAPIDocumentBuilder':
        self._build['info']['license'] = dict(name=name, url=url)
        return self

    def set_external_doc(self, url: AnyUrl, description: t.Optional[str] = None) -> 'OpenAPIDocumentBuilder':
    ...
    def add_server(
        ...
        self._build.setdefault('servers', []).append(dict(url=url, description=description, variables=variables))
        return self
    ...
    def add_tags(
        ...
        if external_doc_url:
            data['externalDocs'] = dict(description=external_doc_description, url=external_doc_url)
        self._build.setdefault('tags', []).append(data)
        return self

    def _get_openapi_route_document_models(self, app: StarletteApp) -> t.List[OpenAPIRoute]:
        openapi_route_models = []
    ...
    def _get_operations_models(self, routes: t.List[OpenAPIRoute]) -> t.List[ModelField]:
        operation_models = []
        for route in routes:
    ...
    def _get_model_definitions(
        ...
        definitions: t.Dict[str, t.Dict[str, t.Any]] = {}
        for model in models:
    ...
    def error_model_fields(
        ...
        for schema in [ValidationError, HTTPValidationError]:
            model_field = create_response_field(
                name=schema.__name__, type_=schema
            )
            _model_fields.append(model_field)
        return _model_fields

    def build_document(self, app: StarletteApp) -> OpenAPI:
        ...
        _flat_model = self._get_operations_models(openapi_route_models) + self.error_model_fields
        ...
        mounts = (_route for _route in openapi_route_models if isinstance(_route, OpenAPIMountDocumentation))
        for item in mounts:
            data = item.get_tag()
            if data:
                self._build.setdefault('tags', []).append(data)
