...
def bootstrap() -> App:
    application = AppFactory.create_from_app_module(
        lazyLoad("ellar_catching.root_module:ApplicationModule"),
        config_module=os.environ.get(
            ELLAR_CONFIG_MODULE, "ellar_catching.config:DevelopmentConfig"
        ),
        global_guards=[],
    )
    ...
    document_builder.set_title("Ellar Caching Tutorial").set_version(
        "1.0.2"
    ).set_contact(
        name="Author Name",
