...
def test_import_project_meta_returns_default_when_py_project_is_none(tmpdir):
    os.chdir(tmpdir)
...
def test_cwd_has_pyproject_file_returns_false(tmp_py_project_path):
    assert EllarCLIService.cwd_has_pyproject_file() is False


def test_create_ellar_project_meta_work(
    tmpdir, write_empty_py_project, tmp_py_project_path
):
...
def test_create_ellar_project_meta_fails_if_py_project_does_not_exist(
    tmpdir, tmp_py_project_path, mock_py_project_table
):
    cli_service = EllarCLIService(
        py_project_path=tmp_py_project_path,
...
def test_create_ellar_project_meta_fails_if_project_name_exist(
    tmpdir, tmp_py_project_path, add_ellar_project_to_py_project
):
...
def test_import_application_works(tmpdir, write_empty_py_project, cli_runner):
    result = cli_runner.invoke_ellar_command(["create-project", "new_project_one"])
...
def test_import_configuration_works(tmpdir, write_empty_py_project, cli_runner):
    result = cli_runner.invoke_ellar_command(["create-project", "new_project_two"])
    assert result.exit_code == 0
...
def test_import_root_module_works(tmpdir, write_empty_py_project, cli_runner):
    result = cli_runner.invoke_ellar_command(["create-project", "new_project_three"])
    assert result.exit_code == 0
...
def test_import_apps_module_works(tmpdir, write_empty_py_project, cli_runner):
    result = cli_runner.invoke_ellar_command(["create-project", "new_project_four"])
    assert result.exit_code == 0
...
def test_get_apps_module_path_works(tmpdir, write_empty_py_project, cli_runner):
    result = cli_runner.invoke_ellar_command(["create-project", "new_project_five"])
    assert result.exit_code == 0

    ellar_cli_service = EllarCLIService.import_project_meta()
    apps_module_path = ellar_cli_service.get_apps_module_path()

    assert str(tmpdir) in apps_module_path
