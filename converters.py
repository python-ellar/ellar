...
def _get_origin(outer_type_: t.Type) -> t.Type:
...
class TypeDefinitionConverter(ABC):
    def __init__(self, outer_type_: t.Type):
    ...
    @abstractmethod
    def get_modified_type(self, outer_type_: t.Type) -> t.Type[t.Any]:
        ...

    def get_sub_fields(self, sub_fields: t.Optional[t.List[t.Any]]) -> t.List['TypeDefinitionConverter']:
        _sub_fields = []
        for field in sub_fields or []:
            _sub_fields.append(self.__class__(field))
        return _sub_fields

    def re_group_outer_type(self) -> t.Type:
