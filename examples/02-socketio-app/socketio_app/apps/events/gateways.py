...
from ellar.socket_io.decorators import WebSocketGateway, subscribe_message, on_connected, on_disconnected
from ellar.socket_io.responses import WsResponse
from .schemas import MessageRoom, MessageData, MessageChatRoom
...
class EventsGateway:
    ...
    async def leave(self, message: MessageRoom = WsBody()):
        ...
        await self.context.server.emit('my_response', {'data': 'Left room: ' + message.room}, room=self.context.sid)

    @subscribe_message('close_room')
    async def close_room(self, message: MessageRoom = WsBody()):
        await self.context.server.emit('my_response', {'data': 'Room ' + message.room + ' is closing.'}, room=message.room)
    ...
    async def my_room_event(self, message: MessageChatRoom = WsBody()):
        await self.context.server.emit('my_response', {'data': message.data}, room=message.room)
